...
(deftest in-vector
  (let [dtc (u/dom-type-collection :properties {:x (s/vector (s/number :default -2))
                                                :y (s/vector (s/number :default f))})
        out (u/insert dtc {:x [] :y [] :company-id (ObjectId.)})]
    (is (= [] (:x out)))
    (is (= [] (:y out)))))
...
(deftest in-map
  (let [dtc (u/dom-type-collection :properties {:x (s/map {:y (s/number :default -2)
                                                           :z (s/number :default f)})})
        out (u/insert dtc {:x {} :company-id (ObjectId.)})]
    (is (= -2 (get-in out [:x :y])))
    (is (= 42 (get-in out [:x :z])))))

(deftest on-map
  (let [dtc (u/dom-type-collection :properties {:x (s/map {:w (s/number)}
                                                          :default {:w -2})
                                                :y (s/map {:z (s/number)}
                                                          :default {:z 42})})
        out (u/insert dtc {:company-id (ObjectId.)})]
...
(deftest in-vector-of-maps
  (let [dtc (u/dom-type-collection :properties {:x (s/vector (s/map {:y (s/number :default -2)
                                                                     :z (s/number :default f)}))})
        out (u/insert dtc {:x [{} {}] :company-id (ObjectId.)})]
...
(deftest in-map-of-vectors-of-maps
  (let [dtc (u/dom-type-collection :properties {:w (s/map {:x (s/vector (s/map {:y (s/number :default -2)
                                                                                :z (s/number :default f)}))})})
        out (u/insert dtc {:w {:x [{} {}]} :company-id (ObjectId.)})]
